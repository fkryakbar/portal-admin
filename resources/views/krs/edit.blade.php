    ...
    <div class="lg:p-5 p-2 min-h-screen">
        <div class="flex justify-between items-center">
            <div class="flex gap-2 items-center text-gray-500  mb-3">
                ...
                <h1 class="font-bold text-2xl">Edit KRS</h1>
            </div>
        </div>
        ...
        <form action="" method="POST" class="mt-5">
            ...
            <div class="mb-6">
                ...
                <select id="matkul" name="kode_mata_kuliah"
                    ...
                    @foreach ($matkul as $i => $m)
                        <option @selected($krs->kode_mata_kuliah == $m->kode) value="{{ $m->kode }}">
                            {{ $m->nama }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-6">
                ...
                <select id="tahun_ajaran" name="tahun_ajaran"
                    ...
                    @foreach ($tahun_ajaran as $t)
                        <option @selected($krs->tahun_ajaran == $t->kode_tahun_ajaran) value="{{ $t->kode_tahun_ajaran }}">{{ $t->nama_tahun_ajaran }}
                        </option>
                    @endforeach
                </select>
            </div>
            <div class="mb-6">
                ...
                <input type="text" id="dosen_ampu" name="dosen_ampu" value="{{ $krs->dosen_ampu }}"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5"
                    placeholder="">
            </div>
            <div class="mb-6">
                <label for="jadwal" class="block mb-2 text-sm font-medium text-gray-900">Jadwal</label>
                <input type="text" id="jadwal" name="jadwal" value="{{ $krs->jadwal }}"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5"
                    placeholder="">
            </div>
            <button type="submit"
